...
export default function HeroSide({ visible, handleSectionClick, onFileChange }) {
  ...
  return visible ? (
    <div className={`flex-row gap-2 px-2 py-16 md:items-start md:text-left sidebar-content content-2`}>
      ...
      <div className='flex flex-row'>
        <button onClick={() => handleSectionClick(1)} className="inline-flex px-5 py-2 mx-4 text-lg text-white bg-black border-0 rounded focus:outline-none hover:bg-indigo-600">Show 1</button>
        <button onClick={() => handleSectionClick(2)} className="inline-flex px-5 py-2 text-lg text-white bg-black border-0 rounded focus:outline-none hover:bg-indigo-600">Show 2</button>
      </div>
    </div>
  ) : null;
}